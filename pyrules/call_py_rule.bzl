...
def _call_py_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    cmd.add(cmd_args([ctx.attrs.py_toolchain[OtlPythonToolchainInfo].interpreter, ctx.attrs.ex_script, "--args", json.encode(ctx.attrs.args), "--output", out.as_output()]))
...
python_caller = rule(
    ...
    attrs = {
        ...
        "otl_libs": attrs.list(attrs.dep(providers = [OtlLibSrcs]), default=["//:otl_lib"]),
