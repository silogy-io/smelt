use bollard::{
    self, container::RemoveContainerOptions, exec::CreateExecOptions, image::CommitContainerOptions,
};
...
use smelt_data::client_commands::{CfgDocker, RunMode};
...
use super::super::common::{create_test_result, get_target_root, handle_line, prepare_workspace};

pub struct SealedExecutor {
    docker_client: Docker,
    /// Name of the base image the docker executor will be using
    image_name: String,
...
#[derive(Clone)]
struct SealedName(String);
trait SealedUtils {
    fn set_sealed_name(&mut self, name: String);
    fn get_sealed_name(&self) -> String;
}

impl SealedUtils for UserComputationData {
    fn set_sealed_name(&mut self, name: String) {
        self.data.set(SealedName(name));
    }
    fn get_sealed_name(&self) -> String {
        self.data.get::<SealedName>().cloned().unwrap().0
    }
}

#[async_trait]
impl Executor for SealedExecutor {
    async fn drop_per_tx_state(&self, _dice_data: &UserComputationData) {
        let tx = _dice_data.get_tx_channel();
        let ctname = _dice_data.get_sealed_name();
        let docker = &self.docker_client;
        let options = CommitContainerOptions {
            container: ctname.clone(),
            pause: true,
            tag: ctname.clone(),
            ..Default::default()
        };

        let maybe_commit = docker
        ...
        let _ = docker.remove_container(ctname.as_ref(), options).await;
    }
    ...
    async fn init_per_tx_state(
        ...
        let container_name = format!("smelt-sealed-{}", container_name_prefix);
        _dice_data.set_sealed_name(container_name.clone());
        ...
        let a = docker
            .start_container(container.id.as_ref(), None::<StartContainerOptions<String>>)
            .await;
        if let Err(err) = a {
            let tx = _dice_data.get_tx_channel();
            tx.send(Event::runtime_error(
                err.to_string(),
                _dice_data.get_trace_id(),
            ))
            .await;
        ...
        let source_path = global_data.get_smelt_root().to_string_lossy().to_string();
        tokio::process::Command::new("docker")
            .arg("cp")
            .arg(format!("{}.", source_path))
            .arg(format!("{}:{}", container_name, source_path))
            .output()
            .await;
    }
    ...
    async fn execute_commands(
        ...
        let _ = tx
            ...
            .await;
        let container_name_clone = dd.get_sealed_name();
        ...
        docker.create_exec(&container_name_clone, exec_opts).await?;
        ...
        let log_task = tokio::spawn(async move {
            ...
            let mut output = docker_clone.logs(&moved, Some(attach_options));
        ...
        let container_name = dd.get_sealed_name();
