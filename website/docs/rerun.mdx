...
To impliment re-running, otl does the following in sequence:
...
and we only tell otl to execute `TEST1_RERUN` and `TEST2_RERUN`
...
and we only tell otl to execute `TEST1_RERUN` and `TEST2_RERUN`, and when we do this, BUILD_RERUN is first executed by the otl execution engine.
