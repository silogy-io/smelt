...
def test_sealed_slurm():
    """ """
    test_list = f"silogy_base.otl.yaml"
    img = "2be49eff4ada"
    ...
    def init_slurm(cfg: ConfigureSmelt) -> ConfigureSmelt:
        ...
        cfg.slurm.dockerws = DockerWorkspace(
            container_name=img, workspace_smelt_root="/tmp/code"
        )
        cfg.slurm.maybe_info = ServerInfo(hostname="100.110.35.82",worker_port=9004,server_port=9004)
        print(cfg)
        print(cfg.slurm)
        return cfg
